...
public:
  ...
  typedef void (*DataTraceCallback)( uint32_t, std::string, std::string);
...
protected:
  ...
  TracedCallback < uint32_t, shared_ptr<const Data>, uint32_t > m_receivedData;
  TracedCallback < uint32_t, std::string, std::string > m_data;
  TracedCallback < uint32_t, std::string, shared_ptr<const lp::Nack> > m_nack;
  /// @endcond
};
