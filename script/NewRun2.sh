...
do
       ...
       # do
	       ...
	       #	echo "python3 raw_b.py "server-$run-$c-$val-" $run"
               python3 raw_b.py "server-$run-$val-0-" "server-$run-$val-1-" $run
               python3 raw_b2.py "server-$run-$val-0-" "server-$run-$val-1-" $run
	       echo "python3 raw_b.py "server-$run-$c-$val-" "server-$run-1-$val-"  $run"
