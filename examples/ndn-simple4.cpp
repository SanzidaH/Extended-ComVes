...
//  ndn::StrategyChoiceHelper::InstallAll("/update", "/localhost/nfd/strategy/comves");
  ...
  consumerHelper.SetAttribute("Frequency", StringValue(std::to_string( std::stoi(freq)))); // 10 interests a second
...
//   apps2.Stop(Seconds(100.0)); // stop the consumer app at 10 seconds mark
  ...
  producerHelper.SetAttribute("Seed", StringValue(std::to_string(run)));
